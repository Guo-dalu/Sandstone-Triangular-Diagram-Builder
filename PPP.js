...
for(var ip = 0; ip<inputs.length; ip++){
	inputs[ip].addEventListener("focus",function(event){
		this.style.fontSize="16px";
	},false);}
var lineinputvalid=function (){
	for(var w = 0; w<inputs.length; w++){
if (!inputs[w].value.replace(/\s+/g," ").trim().length) { warns[w].innerText="请输入数字";}		
else{
	var temp1=inputs[w].value.replace(/\s+/g," ").trim().split(" ");
for (var xl=0;xl<temp1.length;xl++){
		if (!/^\d*\.{0,1}\d{0,1}$/.test(temp1[xl])) {warns[w].innerText="请输入数字，数字不能为负数";break;}
		else if (+(temp1[xl]) < 0||+(temp1[xl])>100) {warns[w].innerText="百分数不应超过100";break;}		
		else if (/^\d*\.{0,1}\d{0,1}$/.test(temp1[xl])&&(+(temp1[xl])<100))
			{warns[w].innerText="";}
		else {warns[w].innerText="请检查数据格式";break;}
	}
	delete temp1;
}		
}
...
var dataareavalid=function(){
	dataarea.addEventListener("blur",function(event){
		var temp2=[];
		temp2=dataarea.value.replace(/\s+/g," ").trim().split(" ");
		console.log(temp2);
		if (temp2.length===0) {warns["txtwarn"].innerText="请输入数字";}
		else {
			for(var xx=0;xx<temp2.length;xx++){
		if (!/^\d*\.{0,1}\d{0,1}$/.test(temp2[xx])) {warns["txtwarn"].innerText="请输入数字，数字不能为负数";break;}
		else if (+(temp2[xx])>100) {warns["txtwarn"].innerText="百分数不应超过100";break;}
...
var showall=function (event){
	...
	{	var temp1=[];values[j]=[];
		temp1=inputs[j].value.toString().replace(/\s+/g," ").trim().split(" ");
...
if (!(values[0]&&values[1]&&values[2])) {warns[4].innerText="请在每一行输入数据";return;}
else if (!(values[0].length===values[1].length&&values[2].length===values[1].length) ){warns[4].innerText="请输入三个一组的成组数据";return;}//每一组3个数据 
...
var showallfromtxtarea =function (event){	
	if (warns[4].innerText!==""||dataarea.value===""){alert("!");return;}
	else{drawoutline();	console.log(" in showall else");	
	warns[4].innerText="";
	for (var pl = 0; pl < stonePercents.length; pl++) {
		stonePercents[pl].innerText="";
	}
	...
	stonetotaldata=[];
	var flag=0;
	var temp1=[];temp2=[];console.log("temp1 before push: "+temp1);	
	temp1=dataarea.value.replace(/\s+/g," ").trim().split(" ");		
	console.log("temp1 after push: "+temp1.length);				
	if (temp1.length%3!==0||temp1.length===0){warns[4].innerText="请输入三个一组的成组数据";flag=1;return;}
	else{
		stonetotaldata=temp1;
...
if(txtinput.style.visibility==="hidden"||txtinput.style.visibility===""||txtinput.style.visibility===null) 
	{	btn.addEventListener("click",lineinputvalid,false);
		if(warns[0].innerText===""&&warns[1].innerText===""&&warns[2].innerText==="")
